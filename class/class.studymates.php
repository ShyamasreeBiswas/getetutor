...
class studymates {
	...
	function getMateName($vid)
	{
		$query = "SELECT mate_name FROM study_materials WHERE id = '".$vid."'";
		$search_array=$this->search($query);
		return  ucfirst($search_array[0]['mate_name']);
	}
	
	function getMateType($vid)
	{
		$query = "SELECT mate_type FROM study_materials WHERE id = '".$vid."'";
		$search_array=$this->search($query);
		return  ucfirst($search_array[0]['mate_type']);
	}
