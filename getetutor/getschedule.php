...
{	
	...
	if($id>0) 
	{
		if($schedules->save($id)) 
		{
			$GLOBALS['admin_msg'] = "Schedule updated successfully";
			disphtml("main();");
		} else {
			$GLOBALS['admin_msg'] = $schedules->admin_msg;
		  	disphtml("saveData(".$id.");");
		}
	} 
	else 
	{
		if($schedules->save($id="NULL")) 
		{
			$GLOBALS['admin_msg'] = "Schedule inserted successfully";
			disphtml("main();");
		} 
		else 
		{
